...
output_video_path = os.path.join(base_dir, "data", "annotated_clip_abandoned_8s.mp4")
vlm_frames_dir = os.path.join(base_dir, "data", "vlm_input_frames_abandoned")
artifact_dir = r"C:\Users\higoi\.gemini\antigravity-ide\brain\5bde3e10-5ab7-412f-9811-185e514054b1\vlm_input_frames_abandoned"
payload_path = os.path.join(base_dir, "vlm_prompt_payload_abandoned.json")

os.makedirs(vlm_frames_dir, exist_ok=True)
os.makedirs(artifact_dir, exist_ok=True)

# 2. Golden segment for Abandoned Object: seconds 44.0 to 52.0 (8.0 seconds)
START_SEC = 44.0
END_SEC = 52.0
NUM_VLM_FRAMES = 8
...
# Use avc1 / OpenH264 for direct browser & IDE playback
...
for order, s_idx in enumerate(sample_indices, 1):
    ...
    fn = f"vlm_abandoned_frame_{order:02d}_{f_sec:.2f}s.jpg"
...
# 6. Generate VLM Prompt Payload for Abandoned Object Scenario
prompt_payload = {
    "task": "Video Visual Relation Detection (VidVRD) - Abandoned Object Scenario",
    "scenario": "Abandoned Object Detection via Human-Human & Human-Object Relation Transitions",
    ...
    "vlm_system_prompt": "You are an advanced Video Visual Relation Detection AI for surveillance analytics. You are given a sequential series of video frames with numbered visual marks [ID] identifying subjects and objects. Your task is to detect all active relations between the marked entities over time. You MUST strictly choose relations from the provided 26 relation categories. Output strictly valid JSON list of triplets: [{\"subject\": \"[ID]\", \"relation\": \"<verb>\", \"object\": \"[ID]\"}].",
    "vlm_user_prompt": f"Analyze the {NUM_VLM_FRAMES} sequential frames of this surveillance clip (44s to 52s). Entities detected with visual marks: {list(tracked_entities.keys())}. 1. Which relations from the 26 categories occur between persons (e.g. touch/hug)? 2. Does any person interact with or abandon the bag on the ground? Respond ONLY with a JSON array of triplets.",
    "abandoned_object_logic_reference": {
        "state_transition": "Active interaction [person, hold/carry, bag] -> No interaction [None] while spatial distance increases -> Abandoned State",
        "abandoned_item": "handbag/backpack",
        "trigger_condition": "Stationary object on floor with no owner within proximity for > threshold time"
    }
}
...
print(f"Abandoned Object VLM Prompt Payload written to: {payload_path}")
print("SUCCESS! ABANDONED OBJECT PIPELINE COMPLETED!")
